...
public/assets/
  ...
  audio/
    music/         # 4 music tracks (forest, cave, village, menu)
    sfx/           # 5 sound effects
    ambient/       # 3 ambient loops
scripts/
  generate-art.ts          # Main art generation pipeline (ComfyUI API)
  generate-menu-theme.py   # Menu music (Python wave synthesis)
...
All audio was generated via Python synthesis — no external audio libraries or AI music tools.

### Menu Theme (generate-menu-theme.py)

Pure Python using only `wave`, `struct`, and `math` (no numpy). Generates a 51-second D minor theme at 112 BPM.

**5 voices:**
| Voice | Waveform | Volume | Role |
|-------|----------|--------|------|
| Lead melody | warm_lead (sine + triangle + overtones) | 0.35 | Main hook |
| Lead chorus | warm_lead (detuned +1.5 Hz) | 0.15 | Thickening/chorus |
| Bass | sine | 0.30 | Root notes |
| Chord pad | sine (triads) | 0.08 | Harmonic bed |
| Arpeggio | pluck (sine + decaying harmonics) | 0.14 | Harp-like texture |

**Structure:** Intro (4 bars) → Theme A (8 bars) → Theme B (8 bars) → Outro (4 bars)

**Effects:**
- Delay on lead: 280ms, feedback 0.25
- Delay on chorus: 350ms, feedback 0.2
- Delay on arpeggio: 200ms, feedback 0.2
- 3-second fade-out
- Normalized to 0.75 peak amplitude

**Output:** Mono 16-bit WAV at 44100 Hz.
...
### Audio
- **Pure Python wave synthesis is surprisingly capable** for retro game audio. No external audio libraries needed for the menu theme — just `wave`, `struct`, and `math`.
...
6. **Generate audio:** Adapt the Python synthesis scripts. Change the key, tempo, and melodies.
